...
Figure \ref{fig:dEdx_brems} shows a comparison of the average energy for electrons loss due to bremsstrahlung when using different differential cross sections.
For very high energies, all parametrizations lead to similar results in the average energy loss.
However, differences of up to a factor $\num{1.5}$ can be seen for lower energies, especially in the domain of several \si{\mega\electronvolt}.
...
This behavior results from the fact that the correction factor $A^{\prime}(E,Z)$ is set to $1$ for $E > \SI{50}{\mega\electronvolt}$ although $A^{\prime}(E,Z) \neq 1$ according to the empirical data.

%
\begin{figure}
    \centering
    \includegraphics[scale=1]{build/dEdx_brems.pdf}
    \caption{Comparison of the average energy loss of electrons in air due to bremsstrahlung between the parametrization adapted from EGS (Electron Screening) and two alternative parametrizations (Complete Screening, Andreev Bezrukov Bugaev). The dashed red line highlights the cut at $\SI{50}{\mega\electronvolt}$ between the high-energy and low-energy part applied in the Electron Screening parametrization.}
