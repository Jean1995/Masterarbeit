...
\end{figure}
%
Electron-positron pair production is the dominant interaction of photons in matter with energies above several $\si{\mega\electronvolt}$ and is therefore essential for the propagation of high energy photons.
...
\end{align}
%
While the approximate differential cross section ignores effects from nuclear form factors, which are only important for large production angles, the atomic form factors are described by the functions $\varphi_1$, $\varphi_2$ (elastic scattering) and $\psi_1$, $\psi_2$ (inelastic scattering).
The description used for the atomic form factors varies with $Z$, the different expressions for $\varphi$ and $\psi$ are given in Appendix \ref{sec:atomic_form_factors}.
