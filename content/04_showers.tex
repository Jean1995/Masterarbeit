...
Furthermore, as a proof of concept, simulations of photon-induced electromagnetic showers with PROPOSAL using the newly implemented cross sections are presented.
...
PROPOSAL uses a modified Bethe formula, taking into account density correction effects, to describe the ionization losses for muon and tau leptons \cite{Kohne:2013zbq, Rossi:99081, PhysRevD.98.030001}.
...
Furthermore, $F^{\pm}(\tau, \Delta)$ differs for electrons ($F^-$) and positrons ($F^+$) and is defined by
\begin{align}
	\begin{split}
...
To take into account the atomic excitation levels relevant for small energy transfers, the Berger-Seltzer formula, i.e.\ \eqref{eqn:ioniz_sum}, is used when calculating continuous ionization losses.
When sampling stochastic losses, the differential cross section for M{\o}ller scattering \eqref{eqn:moller}, respectively the differential cross section for Bhabha scattering \eqref{eqn:bhabha}, is used directly since the contributions from small energy transfers are negligible here.
...
While the contribution from atomic excitation levels is, as a good approximation, identical for electrons and positrons \cite{PhysRev.93.38}, the contribution from high energy transfers differs.
...
However, a positron as an ingoing particle is always distinguishable from the atomic electron and can therefore be the particle with the lower energy after the interaction.
...
This parametrization is implemented in PROPOSAL to examine the differences between the cross sections provided by PROPOSAL and EGS as well as effects from low energy corrections.

\subsubsection{Theoretical description}

The bremsstrahlung cross section provided by EGS, which is described in detail by \cite{Hirayama:2005zm}, is split into a high-energy and a low-energy part with a discrete cut at $\SI{50}{\mega\electronvolt}$.
...
PROPOSAL uses a two-dimensional interpolation routine to obtain the correction factor for an arbitrary ($Z$, $E$) from the discrete table values.

\begin{figure}
    \centering
    \includegraphics[scale=1]{build/dEdx_brems.pdf}
    \caption{Comparison of the average energy loss of electrons in air due to bremsstrahlung between the parametrization adapted from EGS (Electron Screening) and two alternative parametrizations (Complete Screening, Andreev Bezrukov Bugaev). The dashed red line highlights the cut applied in the Electron Screening parametrization between the high-energy part and the low-energy part.}
    \label{fig:dEdx_brems}
\end{figure}


Figure \ref{fig:dEdx_brems} shows a comparison of the average energy loss for electrons\footnote{Regarding only leading order calculations in $\alpha$, the bremsstrahlung cross section is identical for electrons and positrons \cite{RevModPhys.46.815}.} loss due to bremsstrahlung when using different parametrizations.
...
This behavior results from the fact that the correction factor $A^{\prime}(E,Z)$ is set to $1$ for $E > \SI{50}{\mega\electronvolt}$ although this approximation is invalid according to empirical data implying that $A^{\prime}(E,Z) \neq 1$ for energies above the cut.
A smoothing of the correction parameter around $\SI{50}{\mega\electronvolt}$ could remove the unphysical discontinuity, however, such a method would be arbitrary to a certain extent.
...
Under the assumption that the atomic electron is initially free and at rest, which is an appropriate approximation for positrons with energies high compared to atomic binding energies, the differential annihilation cross section is described by the Heitler formula \cite{heitler1954quantum} and given by \cite{Hirayama:2005zm}
%
\begin{equation}
...
%
\begin{align*}
	\gamma &= \frac{E}{m_e}, & \epsilon &= \frac{E_{\gamma, 1}}{E + m_e},
\end{align*}
%
with the energy of the initial positron $E$ and the energy $E_{\gamma, 1}$ of one of the created photons.
Here, $\epsilon$ describes the ratio of the energy transfer to one of the created photons to the available energy, i.e.\ the positron energy and the mass of the atomic electron.
...
A general two-body interaction can be described by the process
%
\begin{align*}
	X_1 + X_2 \rightarrow X_3 + X_4
\end{align*}
%
with the corresponding energies $E_i$, masses $m_i$ and absolute momenta $p_i$ of the particles involved.
Applying the conservation of four-momentum (i.e.\ conservation of energy and conservation of 3-space momentum) leads to the relation \cite{Hirayama:2005zm}
%
\begin{equation}
...
%
\begin{equation}
	\frac{1}{\sigma_{\text{stoch}}} \int_{\epsilon_{\text{min}}}^{\epsilon} \frac{\symup{d}\sigma}{\symup{d}\epsilon} \, \symup{d}\epsilon = \xi_1
\end{equation}
%
for $\epsilon$ with a random number $\xi_1 \in \left[0,1\right)$.
...
%
\begin{align}
	\Phi_{\gamma,1} &= 2 \pi \xi_2, & \Phi_{\gamma,2} &= (2 \pi \xi_2 + \pi) \;\mathrm{mod}\; 2 \pi,
\end{align}
%
where $\xi_2 \in \left[0,1\right)$ is an additional random number.
...
Since annihilation interactions are always stochastic with $v=1$, the process is not influenced by the energy cut settings, resulting in stochastic losses with $E \cdot v < {e_\text{cut}}$.
...
However, for energy losses around $\SI{1}{\giga\electronvolt}$, a contribution to the secondary spectrum at a level of a few percents can be observed.
...
Photo pair production, called electron-positron pair production in the further course of this section (although not to be confused with the creation of an electron-positron pair by an ingoing lepton), describes the creation of an electron-positron pair by an ingoing photon in the field of an atomic nucleus.
...
However, the evaluation of this expression is too complicated for a direct implementation.
...
The function $X$, describing the influence of atomic form factors, varies with $Z$ and is defined in appendix \ref{sec:atomic_form_factors}.
...
%
\begin{equation}
	\frac{1}{\sigma_{\text{stoch}}} \int_{x_{\text{min}}}^{x} \frac{\symup{d}\sigma}{\symup{d}x} \, \symup{d}x = \xi_1
\end{equation}
%
for $x$ with a random number $\xi_1 \in \left[0,1\right)$.
...
To offer different levels of accuracy in the simulation of the angular distribution, two options are available to describe the angles between the initial photon and the created electron and positron.
...
\begin{equation}
    ...
    \left( \frac{\symup{d}\sigma}{\symup{d}p}(p^{\ast}) \right)^{-1} \int_{0}^{\theta} \frac{\mathrm{d}^2\sigma}{\mathrm{d}\theta \mathrm{d}p}(p^{\ast}) \, \symup{d}\theta = \xi_2
\end{equation}
%
for $\theta$ where $\xi_2 \in \left[0,1\right)$ is an additional random number and $p = p^{\ast}$ the fixed absolute momentum of the electron, respectively the positron.
...
%
\begin{align}
	\Phi_{-} &= 2 \pi \xi_3, & \Phi_{+} &= (2 \pi \xi_3 + \pi) \;\mathrm{mod}\; 2 \pi,
\end{align}
%
where $\xi_3 \in \left[0,1\right)$ is an additional random number.

Figure \ref{fig:angles} shows a comparison of the two available sampling methods.
For high energies as well as for analyses where the exact distribution of $\theta$ is irrelevant, the simple expression according to \eqref{eqn:angular_egs} provides an adequate description of the production angle.
%
\begin{figure}
    \centering
    \includegraphics[scale=1]{build/photopair_angles.pdf}
    \caption{Distribution of pair production angles in air with an initial particle energy of $E_{\gamma} = \SI{e2}{\electronvolt}$. The vertical line labeled "EGS approximation" shows the production angle according to \eqref{eqn:angular_egs}, the line labeled "Tsai" shows the distribution for the production angle according to \eqref{eqn:angular_tsai} where $\left<\theta\right>$ denotes the mean of this distribution. For this plot, $x=0.85$ has been used. Note that the distribution of Tsai is affected by the choice of $x$, however, the expression according to EGS is independent of $x$. }
    \label{fig:angles}
\end{figure}
...
Per default, both particles inherit the direction of the initial photon, i.e.\ the production angle is neglected.
...
Under the assumption that the binding energy of the atomic electrons can be neglected, which is an adequate approximation especially for photon energies that are high enough to be relevant for shower propagation, i.e.\ $E > 2 m_e$, the differential cross section for Compton scattering is given by the Klein-Nishina formula \cite{KleinNishina}.
...
\end{align*}
%
according to the kinematics for Compton scattering, yields
%
\begin{equation}
...
\begin{figure}
    ...
    \caption{Plot showing the differential Compton Scattering cross section in $\cos(\theta)$ for different photon energies according to the Klein-Nishina formula. The angular coordinate determines the deflection angle of the initial photon, the radial axis describes the differential cross section in arbitrary units.}
    \label{fig:compton}
\end{figure}
...
In a stochastic Compton interaction, the initial photon is deflected by a polar angle $\theta$ according to \eqref{eqn:angle_compton} while the azimuth $\Phi$ is selected randomly in the range $\left[0,2\pi\right)$.
...
With the improvement of electron and positron propagation, as described in section \ref{sec:electronprop}, as well as the implementation of photon propagation, as described in section \ref{sec:photonprop}, PROPOSAL can be used to simulate all particles in an electromagnetic shower.
...
Figures \ref{fig:hex} and \ref{fig:hex_xy} show the track densities of two electromagnetic showers with different initial photon energies.
