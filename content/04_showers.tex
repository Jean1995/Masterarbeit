...
This requires adjustments to the simulation of the stochastic energy loss in the propagation algorithm of PROPOSAL described in section \ref{sec:algorithm}.
...
The function $f(Z)$ describes the Coulomb correction which is given by
%
\begin{align}
...
Since the photon must provide the rest masses each of the electron and the positron, the kinematic limits of the process are given by
%
\begin{align}
...
For high energies, the angles between the photon and the produced leptons are rather small and its directions are dominated by multiple scattering effects rather than the initial production angles.
...
\begin{equation}
	...
	\begin{split}
	\frac{\symup{d}^2\sigma}{\symup{d}{\theta}\symup{d}{p}} &= \frac{2 \alpha^3 E_{-}^2}{\pi E_{\gamma} m_e^4} \sin(\theta) \biggl[ \left( \frac{2 x (1-x)}{(1 + l)^2} - \frac{12 l x (1-x)}{(1 + l)^4} \right) G_2(\infty) \\ &\quad+ \left( \frac{2 x^2 - 2x + 1}{(1 + l)^2} + \frac{4 l x (1-x)}{(1 + l)^4} \right) \left( X - 2 Z^2 f(z)^2 \right) \biggr]
	\end{split}
\end{equation}
%
with
...
The function $X$ is a description of the effects from the atomic form factors, varying with $Z$ and defined in appendix \ref{sec:atomic_form_factors}.
