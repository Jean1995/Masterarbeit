...
The cross sections for interactions of leptons with hadrons are strongly connected with parton distribution functions  which describe the probability to find a specific parton (quark or gluon) with a given fraction $x$ of the nucleon's momentum carried by the parton when the impulse transfer is given by $Q^2$.
To make predictions for the differential cross sections, \cite{Cooper_Sarkar_2011} uses the HERAPDF1.5 data set which provides the parton distribution functions based on deep-inelastic scattering measurements at the HERA experiment.
\todo{Insert in a short sentence what happens between PDF and differential cross section, improve the language and, if necessary, the structure}


The values for $\sfrac{\symup{d}\sigma}{\symup{d}v}$ are provided as a two-dimensional table in $E$ and $v$ with $\num{100}$ entries in $E$ and, for each energy, $\num{110}$ entires in $v$.  
...
\end{align}
%
with the mass of the involved nucleus $m$, the center-of-mass energy $\sqrt{s}$ and the momentum transfer $s$.
For values where $Q^2 < Q_\text{min}^2$, the underlying theory of quantum chromodynamics can not be treated pertubatively anymore, meaning that the predictions for the cross sections are not reliable in this kinematic range. 
