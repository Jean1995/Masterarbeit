...
\begin{figure}
    ...
    \caption{Continuous energy losses of muons in ice. No energy cuts are applied in this plot, hence this plot represents the case where all losses are treated continuously.}
    \label{fig:dEdx}
\end{figure}

In figure \ref{fig:spectrum} and \ref{fig:secondary_number} the stochastic losses for muons propagated in ice are shown, the energy cuts applied here are $e_\text{cut} = \SI{500}{\mega\electronvolt}$ and $v_\text{cut} = \num{0.05}$ while the muons are propagated until they decay.
